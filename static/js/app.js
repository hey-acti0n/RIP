...
async function initCatalog() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    loadCards(q, requestId);
  });
  await loadCards($("#q").value.trim(), getRequestId());
  await refreshCartBadge();
}

async function loadCards(q, requestId) {
  const data = await getJSON(`/api/services?${qp({ q, requestId })}`);
